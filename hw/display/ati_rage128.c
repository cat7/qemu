 ...
 * off-VRAM addressing: a 2D blit whose driver placed its data in AGP
 * memory (the same "upper 32MB = AGP_BASE + offset" convention
 * ati_rage128_card_read32() documents) read straight past local VRAM's
 * bound and got back zero for every pixel -- QuickTime's decoded-frame
 * delivery blit on this card does exactly this once AGP is up, and the
 * result was a fixed, uniform colour (whatever an all-zero source
 * decodes to) instead of the real, changing frame the source buffer
 * genuinely held.
 */
                             ...
                             uint32_t bytes, uint8_t **bounce)
{
    ...
    *bounce = NULL;
    ...
    space = ati_rage128_cmd_space(s);
    if (space == R128_ADDR_LOCAL) {
        return NULL;
    }
    dwords = (bytes + 3) / 4;
    buf = g_malloc0((size_t)dwords * 4);
    ati_rage128_card_read_block(s, offset, buf, dwords, space);
    *bounce = (uint8_t *)buf;
    return *bounce;
}
                               ...
                               uint8_t *bounce, uint32_t bytes)
{
    ...
    space = ati_rage128_cmd_space(s);
    if (space == R128_ADDR_AGP) {
        uint32_t off = offset >= ATI_RAGE128_VRAM_SIZE ?
                       offset - ATI_RAGE128_VRAM_SIZE : offset;
        dma_addr_t bus = (s->regs[R128_AGP_BASE >> 2] & ~0x3fffffu) + off;

        pci_dma_write(PCI_DEVICE(s), bus, bounce, bytes);
    } else if (space == R128_ADDR_PCIGART) {
        uint32_t gart_base = s->regs[R128_PCI_GART_PAGE >> 2] & ~0xfffu;
        uint32_t done = 0;
